#!/usr/bin/env python3
"""
Market Maker Detection & Analysis System
Now with M15 pre-alignment and softer thresholds.
"""
...
DIV_TH   = float(SENS.get("DIVERGENCE_THRESHOLD", 20))  # was 30
MM_CONF  = float(SENS.get("MM_CONF_FOR_IDEA", 40))      # was 50
...
class MMDetector:
    ...
    def analyze_mm_activity(self, symbol: str, spot_data: Dict, futures_data: Dict) -> MarketState:
        ...
        h1_bias  = self._bias_generic(spot_data['prices'], fast=10, slow=20, momentum_thr=0.5)  # ~1h window on 1m data
        h4_bias  = self._bias_generic(spot_data['prices'], fast=40, slow=80, momentum_thr=0.5)  # ~4h window
        ...
        # Setup selection: prefer H1 when strong, else allow EARLY M15 if H1 not opposite
    ...
    def format_alert(self, st: MarketState) -> dict:
        aligned = "✅ ALIGNED" if st.bias_aligned else "⚠️ DIVERGENT"
        mm_line = f"🎯 MM {st.mm_side} ({st.mm_confidence:.0f}% conf)" if st.mm_active else "🔍 No clear MM activity"
        corr = "🔄 DIVERGENCE" if st.divergence else "📊 CORRELATED"
        if st.entry>0 and st.risk_reward>=1.2:
            tag = "H1 setup" if st.setup_mode=="H1" else "EARLY (M15) setup"
            plan = (f"**{tag}**\n"
                    f"**Entry**: ${st.entry:.2f}  |  **Stop**: ${st.stop_loss:.2f}\n"
                    f"**Targets**: ${st.take_profit[0]:.2f} | ${st.take_profit[1]:.2f} | ${st.take_profit[2]:.2f}\n"
                    f"**R:R**: {st.risk_reward:.1f}")
        else:
            plan = "⏸️ No clear entry — wait for better alignment"
        return {
            "title": f"{st.symbol} | M15: {st.m15_bias} | H1: {st.h1_bias} | H4: {st.h4_bias} {aligned}",
            "description": f"{mm_line}\n**Spot**: {st.spot_pressure:+.1f} | **Futures**: {st.futures_pressure:+.1f} {corr}\n\n{plan}",
            "color": 0x2ecc71 if st.h1_bias=="LONG" or (st.setup_mode=="EARLY_M15" and st.m15_bias=="LONG")
                     else (0xe74c3c if st.h1_bias=="SHORT" or (st.setup_mode=="EARLY_M15" and st.m15_bias=="SHORT")
                     else 0x95a5a6)
        }

    # ---------- internals ----------
    def _calc_spot_pressure(self, volumes, prices, trades):
        if len(volumes)<10: return 0.0
        vwap = np.average(prices[-20:], weights=volumes[-20:])
        current = prices[-1]
        if not trades:
            return ((current - vwap)/vwap)*100
        sz = [t['size'] for t in trades] or [1.0]
        thr = np.mean(sz) + 1.5*np.std(sz)  # slightly easier than 2σ
    ...
    def _calc_futures_pressure(self, open_interest, funding_rate, volumes):
        ...
        funding_pressure = -funding_rate*800  # slightly less weight than before
        if len(volumes)>10:
    ...
    def _detect_divergence(self, s_press, f_press, prices):
        ...
        if diff>DIV_TH:
            if s_press>15 and f_press<-15: return True,"BULLISH_DIV"
            if s_press<-15 and f_press>15: return True,"BEARISH_DIV"
        if price_trend<-1.5 and s_press>25: return True,"BULLISH_DIV"
        if price_trend> 1.5 and s_press<-25: return True,"BEARISH_DIV"
        return False,"NONE"
    ...
    def _identify_mm_pattern(self, spot_data, futures_data, divergence, div_type):
        ...
        if divergence:
            conf += 25
            if side=="NEUTRAL": side = "ACCUMULATE" if div_type=="BULLISH_DIV" else "DISTRIBUTE"
        # funding edge (milder)
        if abs(futures_data.get('funding_rate',0))>0.006: conf += 10
        active = conf > MM_CONF
        if not active: side="NEUTRAL"
        return {"active": active, "side": side, "confidence": min(conf,100)}

    def _bias_generic(self, prices, fast:int, slow:int, momentum_thr:float):
        if len(prices)<max(slow, fast)+1: return "NEUTRAL"
        sma_f, sma_s = np.mean(prices[-fast:]), np.mean(prices[-slow:])
        mom = (prices[-1]-prices[-slow])/prices[-slow]*100
        if sma_f>sma_s and mom>momentum_thr:   return "LONG"
        if sma_f<sma_s and mom<-momentum_thr:  return "SHORT"
        return "NEUTRAL"
    ...
    def _trade_setup(self, current_price, m15_bias, h1_bias, levels, mm_pattern):
        ...
        if h1_bias=="LONG" and mm_pattern['active'] and mm_pattern['side'] in ("ACCUMULATE","NEUTRAL"):
            entry = current_price*0.998; stop = levels['support']*0.996
        ...
        if h1_bias=="SHORT" and mm_pattern['active'] and mm_pattern['side'] in ("DISTRIBUTE","NEUTRAL"):
            entry = current_price*1.002; stop = levels['resistance']*1.004
        ...
        if USE_M15 and mm_pattern['active'] and h1_bias!="SHORT" and m15_bias=="LONG":
            entry = current_price*0.999; stop = levels['support']*0.996
        ...
        if USE_M15 and mm_pattern['active'] and h1_bias!="LONG" and m15_bias=="SHORT":
            entry = current_price*1.001; stop = levels['resistance']*1.004
