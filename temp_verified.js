        ...
        function updateMarket(price, volume) {
            ...
            // Calculate trend
            const priceHistory = state.prices.slice(-20);
            if (priceHistory.length >= 20) {
                const avgStart = priceHistory[0];
                const avgEnd = priceHistory[priceHistory.length - 1];
                if (avgEnd > avgStart * 1.005) {
                    state.trend = 'BULLISH';
                } else if (avgEnd < avgStart * 0.995) {
                    state.trend = 'BEARISH';
                } else {
                    state.trend = 'NEUTRAL';
                }
        ...
        // Track last signal sent to Discord to prevent duplicates
        let lastDiscordSignalSymbol = null;
        let lastDiscordSignalTime = 0;
        ...
        function updateMarketInfo() {
            ...
            document.getElementById('marketTrend').textContent = state.trend;
            document.getElementById('marketTrend').className =
                `info-value ${state.trend === 'BULLISH' ? 'value-up' : state.trend === 'BEARISH' ? 'value-down' : 'value-neutral'}`;
        ...
        async function scanWatchlistCandidates() {
            ...
            try {
                ...
                // Sort by score and take top 5 for watchlist
                candidates.sort((a, b) => b.score - a.score);
                renderWatchlistItems(candidates.slice(0, 5));

                // ### FIXED: Generate live signals for top 1 candidate (Free Version) ###
                const topSignals = candidates.slice(0, 1).map(c => {
                ...
                if (topSignals.length > 0) {
                    ...
                    // Deduplication Logic: Check if we sent this exact symbol in the last 5 minutes
                    const shouldSend = !lastDiscordSignalSymbol ||
                        topSignals[0].symbol !== lastDiscordSignalSymbol ||
                        (Date.now() - lastDiscordSignalTime > 5 * 60 * 1000);

                    if (shouldSend) {
                        lastDiscordSignalSymbol = topSignals[0].symbol;
                        lastDiscordSignalTime = Date.now();
                ...
                // Discord Notification for Watchlist (Orange Card) -> Only if top 3 changed?
                // For simplicity, we send if we have a robust list, but maybe throttle it?
                // Depending on user request "watchlist on the other orange card"
                // Let's send the top 5 watchlist items
        ...
        function loadTradingViewWidget() {
            const container = document.getElementById('tvEmbed');
            if (!container) return;
            container.innerHTML = `
            ...
            script.text = JSON.stringify({
                ...
                overrides: {
                    ...
                    "mainSeriesProperties.candleStyle.downColor": "#9c27b0",
                    "mainSeriesProperties.candleStyle.borderUpColor": "#0ecb81",
                    "mainSeriesProperties.candleStyle.borderDownColor": "#9c27b0",
                    "mainSeriesProperties.candleStyle.wickUpColor": "#0ecb81",
                    "mainSeriesProperties.candleStyle.wickDownColor": "#9c27b0"
                }
            });
        ...
        (function initPaperTrading() {
            ...
            function loadPaperPortfolio() {
                try {
                    const saved = localStorage.getItem('srus_paper_portfolio');
                    if (saved) {
            ...
            function savePaperPortfolio() {
                try {
                    localStorage.setItem('srus_paper_portfolio', JSON.stringify(paperPortfolio));
                } catch (e) {
            ...
            window.paperSetOrderType = function (type, clickedBtn) {
                console.log('Setting order type to:', type);
                document.querySelectorAll('.paper-order-tab').forEach(t => t.classList.remove('active'));
                clickedBtn.classList.add('active');
                paperCurrentOrderType = type;
                const limitGroup = document.getElementById('paperLimitPriceGroup');
                if (limitGroup) {
            ...
            window.paperSetTradeType = function (type, clickedBtn) {
                console.log('Setting trade type to:', type);
            ...
            document.addEventListener('click', function (e) {
                ...
                // 3. Position Tabs (Open/Pending/Closed)
                ...
                if (posTab) {
                    ...
                    paperCurrentTab = posTab.dataset.tab;
                    renderPaperPositions();
                    return;
                }

                // 4. Submit Button
                if (e.target.id === 'paperSubmitBtn') {
                    e.preventDefault();
                    executePaperTrade();
                    return;
                }
            ...
            // Position tabs
            document.querySelectorAll('.paper-positions-tab').forEach(tab => {
                tab.addEventListener('click', () => {
                    document.querySelectorAll('.paper-positions-tab').forEach(t => t.classList.remove('active'));
                    tab.classList.add('active');
                    paperCurrentTab = tab.dataset.tab;
                    renderPaperPositions();
                });
            });
            ...
            window.paperSetPercent = function (percent) {
                ...
                if (state.price && amountInput) {
                    ...
                    // Now setting amount in USD, so directly use percentage of balance
                    const usdAmount = availableBalance * (percent / 100);
                    amountInput.value = usdAmount.toFixed(2); // Display USD amount
                    updatePaperOrderSummary();
                    updatePaperSubmitButton();
                }
            };
                        ...
                        return `
                            <div class="paper-position-item" style="border-left-color: var(--text-secondary);">
                                <div class="paper-position-row">
                                    ...
                                    <div>
                                        <div class="paper-position-label">P&L</div>
                                        <div class="paper-position-value" style="color: ${(trade.pnl || 0) >= 0 ? 'var(--accent-green)' : 'var(--accent-purple)'}">
            ...
            window.closePaperTrade = function (tradeId) {
                ...
                // Use loose equality (==) to handle string/number mismatch from HTML attribute
                const trade = paperPortfolio.trades.find(t => t.id == tradeId);
                ...
                if (trade.status !== 'open') {
                    console.warn('Trade is not open:', trade.status);
                    return;
                }
                ...
                // Return original "margin"/cost + PnL
                // Cost was: trade.amount * trade.price
                const originalCost = trade.amount * trade.price;

                paperPortfolio.balance += (originalCost + pnl);
                ...
                // Notify Discord (Green for profit, Red for loss)
                const user = getUsername();
                const isWin = pnl >= 0;
                ...
                sendToDiscord({
                    username: "Snipers Leaderboard",
                    embeds: [{
                        title: isWin ? `💰 Trade Closed (Profit)` : `🔻 Trade Closed (Loss)`,
                        color: isWin ? 3066993 : 15158332, // Green or Red
                        description: `**${user}** closed **${trade.type.toUpperCase()}** on **${trade.symbol}**`,
                        fields: [
                            { name: "PnL", value: `${isWin ? '+' : ''}$${pnl.toFixed(2)}`, inline: true },
                            { name: "Entry", value: `$${trade.price.toLocaleString()}`, inline: true },
                            { name: "Exit", value: `$${currentPrice.toLocaleString()}`, inline: true }
                        ],
                        footer: { text: "Paper Trading Competition" },
                        timestamp: new Date().toISOString()
                    }]
            ...
            window.cancelPaperOrder = function (tradeId) {
                const trade = paperPortfolio.trades.find(t => t.id === tradeId);
                if (!trade || trade.status !== 'pending') return;

                paperPortfolio.trades = paperPortfolio.trades.filter(t => t.id !== tradeId);
                savePaperPortfolio();
                updatePaperUI();
                renderPaperPositions();
            };
            ...
            // Also update when state.price changes
            let lastPrice = state.price;
            setInterval(() => {
                if (state.price && state.price !== lastPrice) {
            ...
            // Update paper trading chart when pair changes
            function updatePaperTradingChart() {
                const iframe = document.getElementById('paperTradingIframe');
                if (!iframe) return;

                const tvSymbol = state.tvSymbol || 'OKX:BTCUSDT.P';
                const encodedSymbol = encodeURIComponent(tvSymbol);

                iframe.src = `https://www.tradingview.com/widgetembed/?frameElementId=tradingview&symbol=${encodedSymbol}&interval=15&theme=dark&style=1&timezone=Etc%2FUTC&studies=%5B%5D&hideideas=1&overrides=%7B%22mainSeriesProperties.candleStyle.upColor%22%3A%22%230ecb81%22%2C%22mainSeriesProperties.candleStyle.downColor%22%3A%22%239c27b0%22%2C%22mainSeriesProperties.candleStyle.borderUpColor%22%3A%22%230ecb81%22%2C%22mainSeriesProperties.candleStyle.borderDownColor%22%3A%22%239c27b0%22%2C%22mainSeriesProperties.candleStyle.wickUpColor%22%3A%22%230ecb81%22%2C%22mainSeriesProperties.candleStyle.wickDownColor%22%3A%22%239c27b0%22%7D`;
            }
