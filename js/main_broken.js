...
async function scanWatchlistCandidates() {
    ...
    try {
        ...
        if (topSignals.length > 0) {
            ...
            // Check if different symbol OR more than 60 mins since last send for same symbol
            const isNewSymbol = currentSymbol !== lastDiscordSignalSymbol;
            const isTimeExpired = (currentTime - lastDiscordSignalTime) > 60 * 60 * 1000;

            if (isNewSymbol || isTimeExpired) {
                lastDiscordSignalSymbol = currentSymbol;
                lastDiscordSignalTime = currentTime;
...
(function initPaperTrading() {
    ...
    window.setPaperOrderType = function (type, clickedBtn) {
        ...
        console.log('[DEBUG] limitContainer:', limitContainer, 'marketContainer:', marketContainer);

        if (limitContainer) {
            limitContainer.style.display = type === 'limit' ? 'block' : 'none';
            console.log('[DEBUG] limitContainer display set to:', limitContainer.style.display);
        }
        if (marketContainer) {
            marketContainer.style.display = type === 'market' ? 'block' : 'none';
            console.log('[DEBUG] marketContainer display set to:', marketContainer.style.display);
        }
