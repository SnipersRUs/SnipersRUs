...
def get_tradingview_link(symbol):
    ...
    # Convert BTC/USDT to BTCUSDT for MEXC
...
def vector_sniper_signals(high, low, close, volume, atr_val):
    ...
    # ATR Z-score
    tr = high[-1] - low[-1]
    tr_z = (tr - np.mean(high[-20:] - low[-20:])) / np.std(high[-20:] - low[-20:]) if np.std(high[-20:] - low[-20:]) > 0 else 0
...
class AdaptiveScanner:
    ...
    def scan_symbol(self, symbol):
        ...
        # Cap confidence at 100% max
        confidence = min(100, confidence)
        
        # Minimum 75% confidence for any signal (much more strict)
        if confidence < 75:
            return None
        
        # Determine trade direction - avoid conflicts
        ...
        # Only proceed if we have a clear direction (no conflicts)
        if bullish_signals and not bearish_signals:
            ...
            # Conflicting signals - skip this symbol
            return None
    ...
    def adaptive_scan(self):
        ...
        # Check if we've already sent 3 signals this hour
        ...
        if opportunities:
            ...
            # Filter by quality - only return if we haven't sent 3 signals yet
            high_quality = [o for o in opportunities if o['score'] >= 70]
            watchlist = [o for o in opportunities if 50 <= o['score'] < 70]
            
            # Only return opportunities if we haven't hit the 3 signal limit
            if self.signals_sent_this_hour < 3:
                return {
            ...
            # No opportunities - keep scanning every hour
            self.scan_interval = 3600  # Always 1 hour
            self.no_signal_count += 1
            
            # Silent scanning - no notifications for no opportunities
            return None
...
def main():
    ...
    post_discord("", {
        ...
        "description": f"Scanning top {len(TOP_MACRO_COINS)} macro coins + more",
        "color": 0x00bfff,
        "fields": [
            {
                "name": "📊 Mode",
                "value": "Adaptive Scanning",
                "inline": True
            },
            {
                "name": "⏰ Frequency",
            ...
            {
                "name": "🎯 Focus",
                "value": "Support/Resistance Scalps",
                "inline": True
            }
        ],
        "timestamp": datetime.now(TZ_NY).isoformat()
    ...
    while True:
        try:
            ...
            if scan_result:
                ...
                # Process high quality trades (7-10/10) - MAX 3 trades per hour
                ...
                # Show watchlist (5-7/10) - Only if we haven't sent 3 signals yet
