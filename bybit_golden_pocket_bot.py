    ...
    async def analyze_golden_pocket(self, symbol: str) -> Optional[TradeSignal]:
        ...
        try:
            ...
            if current_price < recent_high and current_price > recent_low:
                ...
                if 0.618 <= retracement <= 0.65:  # Golden Pocket
                    ...
                    # Generate signal
                    if rsi_current < 50 and volume_spike > 1.5:  # Bullish divergence
                        signal = TradeSignal(
                            symbol=symbol,
                            direction='long',
                            ...
                            confidence=0.8,
                            reason=f"Golden Pocket Long - {retracement:.1%} retracement",
                            timestamp=time.time()
                        )
                        return signal
                    
                    elif rsi_current > 50 and volume_spike > 1.5:  # Bearish divergence
                        signal = TradeSignal(
                            symbol=symbol,
                            direction='short',
                            entry_price=current_price,
                            ...
                            leverage=get_symbol_leverage(symbol),
                            confidence=0.8,
                            reason=f"Golden Pocket Short - {retracement:.1%} retracement",
                            timestamp=time.time()
                        )
                        return signal
