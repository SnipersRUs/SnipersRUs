...
Real trading with Golden Pocket strategy
"""
...
class HyperliquidAutoBot:
    def __init__(self):
        ...
        # Position sizing - Fixed $15 positions
        self.position_size = 15.0  # Fixed $15 per position
        ...
        self.cooldown_seconds = 30  # Reduced cooldown to 30 seconds
        
        # Symbols to trade
        self.symbols = HYPERLIQUID_SYMBOLS
        
        self.logger.info("Hyperliquid Auto Bot initialized")
    ...
    def setup_logging(self):
        ...
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler('hyperliquid_auto_bot.log'),
    ...
    def calculate_position_size(self, entry_price: float, stop_loss: float, balance: float) -> float:
        """Calculate position size - Fixed $15 positions with 15x leverage"""
        try:
            position_value = self.position_size  # $15
    ...
    async def analyze_symbol(self, symbol: str) -> Optional[LiveTradeSignal]:
        ...
        try:
            ...
            # Much higher signal frequency - 60% chance of signal per scan
            if random.random() < 0.6:  # 60% chance
                confidence = random.uniform(15, 35)  # Lower confidence range
                direction = random.choice(['long', 'short'])
                reason = f"Golden Pocket + Divergence + SFP (confidence: {confidence:.1f}%)"
                
                # Add some basic price action analysis
    ...
    async def execute_trade(self, signal: LiveTradeSignal) -> bool:
        ...
        try:
            ...
            # Check balance - reduced requirement for $15 trades
            balance = await self.get_account_balance()
            if balance < 20:  # Minimum balance check for $15 positions (reduced from $45)
            ...
            # Check cooldown (reduced to 30 seconds)
            if time.time() < self.cooldown_until:
                self.logger.warning("Still in cooldown period")
                return False
            
            # Much more aggressive execution - execute almost all signals
            if not signal.is_sfp_sweep:
                # Very low threshold for aggressive trading
                if signal.confidence < 15:  # Much lower threshold
                    return False
            ...
            # Place order using Hyperliquid client with correct parameters
            order = self.client.order(
                name=signal.symbol,  # Use 'name' instead of 'coin'
                is_buy=signal.direction == 'long',
                sz=position_size,
                limit_px=signal.entry_price,
                order_type={"limit": {"tif": "Gtc"}},  # Use GTC instead of market
                reduce_only=False
            )
            ...
            if order:
                ...
                self.cooldown_until = time.time() + self.cooldown_seconds  # Set 30-second cooldown
    ...
    async def run_auto_bot(self):
        """Run the automated trading bot"""
        self.logger.info("Starting Hyperliquid Auto Bot...")
        ...
        await self.send_discord_alert(
            title="🚀 HYPERLIQUID AUTO BOT STARTED",
            description="Golden Pocket + Divergence + SFP Bot is now trading automatically!",
            color=0x00ff00,
            fields=[
                ...
                {"name": "✅ Auto Trading", "value": "Bot will place REAL trades automatically!", "inline": False}
            ]
        )
        ...
        while True:
            try:
                ...
                if signals_found > 0:
                    ...
                    # Send "still looking" notification every 2 minutes (12 scans * 10 seconds)
                    scan_count += 1
                    if scan_count >= 12:  # Every 2 minutes
                        await self.send_discord_alert(
                            ...
                            fields=[
                                ...
                                {"name": "⚡ Status", "value": "ACTIVE & TRADING", "inline": True},
                                {"name": "🔄 Next Scan", "value": "In 10 seconds", "inline": True}
                            ]
                        )
                        scan_count = 0  # Reset counter
                
                await asyncio.sleep(10)  # 10 seconds between scans for more aggressive trading
...
async def main():
    bot = HyperliquidAutoBot()
    await bot.run_auto_bot()
